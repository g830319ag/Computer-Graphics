...
void main() {
  ...
  vec3 normal = normalize(texture(normalTexture, textureCoordinate).xyz * 2.0f - 1.0f); // normal to [-1, 1]
  
  float ks = 0.75;
  float kd = 0.75;
  float shininess = 8.0;

  vec3 halfwayVec = normalize(viewDirectionection + fs_in.lightDirection);
  specular = ks * pow(max(dot(normal, halfwayVec), 0.0), shininess);
  diffuse = kd * max(dot(normal, fs_in.lightDirection), 0.0);
