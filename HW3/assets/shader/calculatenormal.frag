...
void main() {
  ...
  // shift 
  vec2 dx = vec2(delta/2, 0.0);
  vec2 dy = vec2(0.0, delta/2);
  
  // gnerate normal vector with sine wave
  vec3 p0 = vec3(fragCoord-dx-dy, sin(offset - 0.1*fragCoord.y));
  vec3 p1 = vec3(fragCoord+dx-dy, sin(offset - 0.1*(fragCoord.y+delta)));
  vec3 p2 = vec3(fragCoord-dx+dy, sin(offset - 0.1*(fragCoord.y+delta)));
  vec3 p3 = vec3(fragCoord+dx+dy, sin(offset - 0.1*(fragCoord.y+delta)));

  // calculate two normals of two triangles and average, then tansfer into [0, 1]
