...
void main() {
  ...
  vec3 N = normalize(vec3(modelMatrix * vec4(normal_in, 1.0)));
  mat3 TBN = mat3(T, B, N); // no need inverse the matrix
  ...
  float height = texture(heightTexture, textureCoordinate_in).x;
  vec3 displacementVec = normal_in * (height * depthScale);
  ...
  gl_Position = viewProjectionMatrix * (modelMatrix * vec4(position_in + displacementVec, 1.0));
}
