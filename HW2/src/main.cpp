...
void loadModels() {
  ...
  float plane_w = 8.192f; // width
  float plane_d = 0.5f; // depth
  float plane_h = 5.12f; // height
  float center_x = 0.0f;
  float center_y = 0.0f;
  float center_z = 0.0f;
  ...
  plane->positions = {
      /*               
                 ...
                 8|/-----|/ 7
      */
      // upper plane (clockwise)
      -plane_w/2 + center_x, center_y, plane_h/2 + center_z, // 1
      plane_w/2 + center_x, center_y, plane_h/2 + center_z, // 2
      plane_w/2 + center_x, center_y, -plane_h/2 + center_z, // 3
      -plane_w/2 + center_x, center_y, -plane_h/2 + center_z, // 4

      // bottom plane (Counterclockwise)
      -plane_w/2 + center_x, -plane_d/2 + center_y, -plane_h/2 + center_z, // 8
      plane_w/2 + center_x, -plane_d/2 + center_y, -plane_h/2 + center_z, // 7
      plane_w/2 + center_x, -plane_d/2 + center_y, plane_h/2 + center_z, // 6
      -plane_w/2 + center_x, -plane_d/2 + center_y, plane_h/2 + center_z, // 5
  ...
  // upper
  plane->texcoords = {0.0f, 0.0f, plane_w/textureSizeX, 0.0f, plane_w/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY};
  // bottom
  plane->texcoords.insert(plane->texcoords.end(), {0.0f, 0.0f, plane_w/textureSizeX, 0.0f, plane_w/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY});
  // front
  plane->texcoords.insert(plane->texcoords.end(), {0.0f, 0.0f, plane_w/textureSizeX, 0.0f, plane_w/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY});
  // right
  plane->texcoords.insert(plane->texcoords.end(), {0.0f, 0.0f, plane_h/textureSizeX, 0.0f, plane_h/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY});
  // back
  plane->texcoords.insert(plane->texcoords.end(), {0.0f, 0.0f, plane_w/textureSizeX, 0.0f, plane_w/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY});
  // left
  plane->texcoords.insert(plane->texcoords.end(), {0.0f, 0.0f, plane_h/textureSizeX, 0.0f, plane_h/textureSizeX, plane_d/textureSizeY, 0.0f, plane_d/textureSizeY});
  ...
  //
  plane->numVertex = m->positions.size();  // 24 points
...
void setupObjects() {
  ...
  ctx.objects.push_back(new Object(3, glm::translate(glm::identity<glm::mat4>(), glm::vec3(4.096, 0 ,2.56))));
...
int main() {
   ...
   *         Ex. HW2 - 312550000
   */
  glfwSetWindowTitle(window, "HW2 - 410551026");
